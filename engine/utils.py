...
def rollout(env_dict, policy, num_env_rollouts, horizon=None, return_wandb_video=True,
            success_vid_first=False, fail_vid_first=False, connect_points_with_line=False, dist=None):
    policy.eval()
    all_env_indices = []
    all_env_rewards = []
    all_env_succ = []
    ...
    for env_description, (env_idx, env) in env_dict.items():
        ...
        additional_metrics = {}
        if dist.get_rank() == 0:
            print(f"Resetting environment for env_idx {env_idx}", flush=True)

        obs = env.reset()

        if dist.get_rank() == 0:
            print(f"Environment reset returned obs for env_idx {env_idx}", flush=True)
            with open("debug_log.txt", "a") as log_file:
                log_file.write(f"Environment reset returned obs for env_idx {env_idx}: {obs}\n")
        for _ in tqdm(range(num_env_rollouts)):
            if dist.get_rank() == 0:
                print(f"Starting rollout iteration {_}", flush=True)
            reward = None
            success = False
            last_info = None
            episode_frames = []
            ...
            if dist.get_rank() == 0:
                if isinstance(obs, dict):
                    ...
                    print(f"Obs type: {type(obs)}, obs: {obs}", flush=True)

                # Write to file as well
                with open("debug_log.txt", "a") as log_file:
                    log_file.write(f"Obs after env reset: {obs}\n")
            # print(f"obs from env reset is {obs}")
            policy.reset()
            done = False
            step_i = 0
            while not done and (horizon is None or step_i < horizon):
                ...
                if dist.get_rank() == 0:
                    print(f"Step completed, reward: {r}, done: {done}, info: {info}", flush=True)
                    with open("debug_log.txt", "a") as log_file:
                        log_file.write(f"Step: reward={r}, done={done}, info={info}\n")

                # Verify observation after step
                if torch.isnan(obs["image"]).any() or torch.isinf(obs["image"]).any():
                    print(f"Invalid observation after step", flush=True)
            ...
            all_succ += success
            print(f"Completed rollout iteration {_}", flush=True)
            with open("debug_log.txt", "a") as log_file:
                log_file.write(f"Completed rollout iteration {_}\n")
    ...
    if dist.get_rank() == 0:
        print("#################### Ending Rollout ####################", flush=True)
        with open("debug_log.txt", "a") as log_file:
            log_file.write("Ending rollout function\n")
