...
def main(cfg: DictConfig):
    ...
    for epoch in metric_logger.log_every(range(cfg.epochs), 1, ""):
        ...
        if epoch % cfg.save_freq == 0:
            ...
            results = rollout(rollout_env, model, 20 // cfg.env_cfg.vec_env_num, horizon=rollout_horizon, dist=dist)
            fabric.barrier()
...
def save_batches(dataloader, num_batches=5):
    # Save a few batches of obs to a file
    track_obs_batches = []
    task_emb = []
    for i, (_, track_obs, _, task_emb, _, _) in enumerate(dataloader):
        if i >= num_batches:
            break
        track_obs_batches.append(track_obs)
    
    torch.save(track_obs_batches, "track_obs.pth")
    torch.save(task_emb, "task.pth")
    print(f"Saved {num_batches} batches of 'obs' to track_obs.pth")
