...
def sample_tracks_object(tracks, num_samples=16, uniform_ratio=0.25, vis=None, motion=False, h=None, object_boxes=None):
    """
    tracks: (T, N, 2)
    num_samples: int, number of samples to take
    uniform_ratio: float, ratio of samples to take uniformly vs. according to displacement
    object_boxes: list of bounding boxes (num_objects, 4), each box is [xmin, ymin, xmax, ymax]
    return: (T, num_samples, 2)

    sample num_samples tracks from the tracks tensor, using both uniform sampling and sampling according
    to the track displacement, and only keeping tracks that start within the given bounding boxes.
    """

    t, n, c = tracks.shape
    print(f"origin track shape is {tracks.shape}")
    ...
    sampled_tracks = tracks[:, idx]
    print(f"shape sampled_tracks is {sampled_tracks.shape}")
    if vis is not None:
        sampled_vis = vis[:, idx]
        # return sampled_tracks, sampled_vis

    if object_boxes is not None:
        # object_boxes shape: (num_objects, 4) -> each box is [xmin, ymin, xmax, ymax]
        valid_mask = torch.zeros(sampled_tracks.shape[1], dtype=torch.bool)

        for box in object_boxes:
            xmin, ymin, xmax, ymax = box
            # Check if the first point of each track (tracks[0]) is within the bounding box
            is_in_box = (sampled_tracks[0, :, 0] >= xmin) & (sampled_tracks[0, :, 0] <= xmax) & (sampled_tracks[0, :, 1] >= ymin) & (sampled_tracks[0, :, 1] <= ymax)
            valid_mask = valid_mask | is_in_box  # Keep track if it is in any of the boxes

        # Apply the mask to filter tracks
        sampled_tracks = sampled_tracks[:, valid_mask]  # Only keep tracks starting within the boxes
        if vis is not None:
            sampled_vis = sampled_vis[:, valid_mask]  # If vis exists, apply the same mask
            return sampled_tracks, sampled_vis
...
def sample_tracks_object_1(tracks, num_samples=16, uniform_ratio=0.25, vis=None, motion=False, h=None, object_boxes=None):
    """
    tracks: (T, N, 2)
    num_samples: int, number of samples to take
    uniform_ratio: float, ratio of samples to take uniformly vs. according to displacement
    ...
    sample num_samples tracks from the tracks tensor, using both uniform sampling and sampling according
    to the track displacement, and only keeping tracks that start within the given bounding boxes.
    Tracks outside the boxes will be removed, but the final shape will remain consistent with the requested `num_samples`.
    """

    t, n, c = tracks.shape
    print(f"origin track shape is {tracks.shape}")

    # Apply motion mask if motion is True
    if motion:
        mask = (tracks > 0) & (tracks < h)
        mask = mask.all(dim=-1)  # if any of u, v is out of bounds, then it's false
        mask = mask.all(dim=0)   # if any of the points in the track is out of bounds, then it's false
    ...
    if num_disp == 0:
        ...
        disp[disp >= threshold] = 1
        disp = disp.to("cuda")
        num_disp = torch.tensor(num_disp, device="cuda")
        disp_idx = torch.multinomial(disp, num_disp, replacement=True)
        uniform_idx = uniform_idx.to("cuda")

        idx = torch.cat([uniform_idx, disp_idx], dim=-1)
    
    sampled_tracks = tracks[:, idx.to(tracks.device)]
    print(f"shape sampled_tracks is {sampled_tracks.shape}")
    if vis is not None:
        sampled_vis = vis[:, idx.to(tracks.device)]
    
    # Handle object_boxes masking and ensure output shape consistency
    if object_boxes is not None:
        # object_boxes shape: (num_objects, 4) -> each box is [xmin, ymin, xmax, ymax]
        valid_mask = torch.zeros(sampled_tracks.shape[1], dtype=torch.bool)
        ...
        for box in object_boxes:
            xmin, ymin, xmax, ymax = box
            # Check if the first point of each track (tracks[0]) is within the bounding box
            is_in_box = (sampled_tracks[0, :, 0] >= xmin) & (sampled_tracks[0, :, 0] <= xmax) & \
                        (sampled_tracks[0, :, 1] >= ymin) & (sampled_tracks[0, :, 1] <= ymax)
            valid_mask = valid_mask.to("cuda")
            is_in_box = torch.tensor(is_in_box,device="cuda")
            valid_mask = valid_mask | is_in_box  # Keep track if it is in any of the boxes

        # Apply the mask to filter tracks
        valid_tracks = sampled_tracks[:, valid_mask.to(tracks.device)]  # Only keep tracks starting within the boxes

        # Fill invalid tracks with NaNs or zeros (using zeros here for simplicity)
        # Ensure the final shape is (T, num_samples, 2) by filling the removed tracks with NaNs or 0
        num_invalid = sampled_tracks.shape[1] - valid_tracks.shape[1]
        
        if num_invalid > 0:
            # Create a tensor of zeros (or NaNs) for the invalid positions
            invalid_tracks = torch.zeros((t, num_invalid, c), dtype=sampled_tracks.dtype)
            # Concatenate valid and invalid tracks to maintain the original shape
            valid_tracks = valid_tracks.to("cuda")
            invalid_tracks = invalid_tracks.to("cuda")
            sampled_tracks = torch.cat([valid_tracks, invalid_tracks], dim=1)

            if vis is not None:
                # Fill the corresponding invalid positions in vis
                invalid_vis = torch.zeros((t, num_invalid), dtype=vis.dtype)  # Assuming vis has shape (T, N) 
                invalid_vis = invalid_vis.to("cuda")
            
                sampled_vis = torch.cat([sampled_vis[:, valid_mask.to(tracks.device)], invalid_vis.to(tracks.device)], dim=1)

        print(f"Final sampled_tracks shape: {sampled_tracks.shape}")
        if vis is not None:
            return sampled_tracks, sampled_vis
