...
from atm.utils.flow_utils import sample_tracks_nearest_to_grids,sample_tracks, sample_tracks_object
...
class SaliencyDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        track_ori = torch.stack(all_view_tracks, dim=0)  # v t track_len n 2
        vi_ori = torch.stack(all_view_vis, dim=0)  # v t track_len n
        ...
        for i in range(len(self.views)):
            ...
            for t in range(self.frame_stack):
                ...
                #     image=image,
                #     caption="the gray bowl. the pink-white plate.",
                #     box_threshold=0.25,
                #     text_threshold=0.1,
                # )
                # filtered_indices = [i for i, class_name in enumerate(labels) if class_name != ""]
                # boxes = boxes[filtered_indices]
                track_i_t, vi_i_t = sample_tracks(tracks=track[i, t], vis=vi[i, t], num_samples=128)
                # track_i_t, vi_i_t = sample_tracks_object(tracks=track[i, t], vis=vi[i, t], num_samples=64, object_boxes=boxes)
        ...
        return obs, track_transformer_obs, track, task_embs, actions, extra_states,track_ori ,vi_ori
