...
class BCViLTPolicy(nn.Module):
    ...
    def __init__(self, obs_cfg, img_encoder_cfg, cam_encoder_cfg, language_encoder_cfg, extra_state_encoder_cfg, track_cfg,
        ...
        # 1.1 encode cam_image
        self._setup_cam_encoder(**cam_encoder_cfg)
    ...
    def _setup_cam_encoder(self, network_name, patch_size, embed_size, no_patch_embed_bias):
        self.spatial_embed_size = embed_size
        self.cam_encoders = []
        for _ in range(self.num_views):
            input_shape = self.obs_shapes["rgb"]
            self.cam_encoders.append(eval(network_name)(input_shape=input_shape, patch_size=patch_size,
                                                          embed_size=self.spatial_embed_size,
                                                          no_patch_embed_bias=no_patch_embed_bias))
        self.cam_encoders = nn.ModuleList(self.cam_encoders)

        self.cam_num_patches = sum([x.num_patches for x in self.cam_encoders])
    ...
    def _setup_spatial_positional_embeddings(self):
        ...
        cam_patch_pos_embed = nn.Parameter(torch.randn(1, self.cam_num_patches, self.spatial_embed_size))
        ...
        modality_embed = nn.Parameter(
            torch.randn(1, len(self.image_encoders) + len(self.cam_encoders) + self.num_views + 1, self.spatial_embed_size)
        )  # IMG_PATCH_TOKENS + CAM_PATCH_TOKENS + TRACK_PATCH_TOKENS + SENTENCE_TOKEN

        self.register_parameter("spatial_token", spatial_token)
        self.register_parameter("img_patch_pos_embed", img_patch_pos_embed)
        self.register_parameter("cam_patch_pos_embed", cam_patch_pos_embed)
        ...
        for i, cam_encoder in enumerate(self.cam_encoders):
            modality_idx += [i] * cam_encoder.num_patches
    ...
    @torch.no_grad()
    def preprocess(self, obs, cam_obs, track, action):
        """
        ...
        return obs, cam_obs, track, action
    ...
    def spatial_encode(self, obs, cam_obs, track_obs, task_emb, extra_states, return_recon=False):
        """
        ...
        for view_idx in range(self.num_views):
            cam_encoded.append(
                rearrange(
                    TensorUtils.time_distributed(
                        cam_obs[:, view_idx, ...], self.cam_encoders[view_idx]
                    ),
                    "b t c h w -> b t (h w) c",
                )
            )  # (b, t, num_patches, c)
        # print("img_encoded shape:", img_encoded.shape)
        
        cam_encoded = torch.cat(cam_encoded, -2)  # (b, t, num_patches, c)
        cam_encoded += self.cam_patch_pos_embed.unsqueeze(0)
        # print("cam_encoded shape:", cam_encoded.shape)
        # 将 obs 和 cam_obs 的编码进行合并
        combined_encoded = torch.cat([img_encoded, cam_encoded], -2)
        B, T = combined_encoded.shape[:2]
        ...
        if self.spatial_transformer_use_text:
            ...
            img_track_text_encoded = torch.cat([combined_encoded, track_encoded, text_encoded], -2)  # (b, t, 2*num_img_patch + 2*num_track*num_track_patch + 1, c)
            ...
            img_track_text_encoded = torch.cat([combined_encoded, track_encoded], -2)  # (b, t, 2*num_img_patch + 2*num_track*num_track_patch, c)
    ...
    def forward(self, obs, cam_obs, track_obs, track, task_emb, extra_states):
        """
        ...
        """
        x, recon_track = self.spatial_encode(obs, cam_obs, track_obs, task_emb, extra_states, return_recon=True)  # x: (b, t, 2+num_extra, c), recon_track: (b, v, t, tl, n, 2)
    ...
    def forward_loss(self, obs, cam_obs, track_obs, track, task_emb, extra_states, action):
        """
        Args:
            obs: b v t c h w
            track_obs: b v t tt_fs c h w
            ...
            action: b t act_dim
        """
        # print(f"cam_obs shape in VILT policy input: {cam_obs.shape}")
        # print(f"obs shape in VILT policy input: {obs.shape}")
        # print(f"cam_obs in VILT policy input: {cam_obs}")
        # print(f"obs in VILT policy input: {obs}")  
        obs, cam_obs, track, action = self.preprocess(obs, cam_obs, track, action)
        dist = self.forward(obs, cam_obs, track_obs, track, task_emb, extra_states)
    ...
    def forward_vis(self, obs, cam_obs, track_obs, track, task_emb, extra_states, action):
        """
        Args:
            obs: b v t c h w
            cam_obs: b v t v h w
        ...
        Returns:
        """
        obs, cam_obs, track, _ = self.preprocess(obs, cam_obs, track, action)
        ...
        for view in range(self.num_views):
            ...
            # 可视化 cam_obs
            gt_cam_obs = cam_obs[:1, view, 0, :, :, :]  # 提取当前视角的 cam_obs，形状为 (1, 3, h, w)
            # 还原 cam_obs 到未归一化的形式
            gt_cam_obs = (gt_cam_obs * 255).clamp(0, 255).byte() 
            gt_cam_obs = gt_cam_obs.repeat(1, track_obs_t, 1, 1, 1).cpu().numpy().astype(np.uint8)  # 在时间维度上进行重复
            # combined_gt_cam_obs = (track_obs[:1, view, 0, :, ...] * 0.25 + gt_cam_obs * 0.75).cpu().numpy().astype(np.uint8)
            # 将 combined_gt_cam_obs 转换为 (height, width, channels)
            gt_cam_obs = gt_cam_obs[0,0].transpose(1, 2, 0)
    ...
    def act(self, obs, cam_obs, task_emb, extra_states):
        """
        Args:
            obs: (b, v, h, w, c)
            cam_obs: (b, v, h, w, c)
        ...
        cam_obs = rearrange(cam_obs, "b v h w c -> b v 1 c h w").clone()
        ...
        cam_obs = torch.Tensor(cam_obs).to(device=device, dtype=dtype)
        ...
        if (cam_obs.shape[-2] != self.obs_shapes["rgb"][-2]) or (cam_obs.shape[-1] != self.obs_shapes["rgb"][-1]):
            cam_obs = rearrange(cam_obs, "b v fs c h w -> (b v fs) c h w")
            cam_obs = F.interpolate(cam_obs, size=self.obs_shapes["rgb"][-2:], mode="bilinear", align_corners=False)
            cam_obs = rearrange(cam_obs, "(b v fs) c h w -> b v fs c h w", b=C, v=self.num_views)
        ...
        with torch.no_grad():
            x, rec_tracks = self.spatial_encode(obs, cam_obs, track_obs, task_emb=task_emb, extra_states=extra_states, return_recon=True)  # x: (b, 1, 4, c), recon_track: (b, v, 1, tl, n, 2)
