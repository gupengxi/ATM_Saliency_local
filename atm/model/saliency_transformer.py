...
class SaliencyTransformer(nn.Module):
    """
    ...
    def forward(self, obs, saliency_obs, task_emb, p_img):
        """
        track: (b, tl, n, 2), which means current time step t0 -> t0 + tl
        obs: (b, t, c, h, w), which means the past time step t0 - t -> t0
        task_emb, (b, emb_size)
        """
        assert torch.max(obs) <=1.
        # B, T, _, _ = track.shape
        print(f"check obs shape {obs.shape}")
        
        patches = self._encode_video(obs, p_img)  # (b, n_image, d)
        saliency_obs = saliency_obs.to(torch.float32)
        saliency_obs = saliency_obs.to("cuda")
        saliency_patches = self._encode_video(saliency_obs, p_img)
        print(f"check sa_obs shape {saliency_obs.shape}")
        ...
        x = torch.cat([saliency_patches, patches, text_encoded], dim=1)
        x = self.transformer(x)

        rec_saliency_patches, rec_patches = x[:, :self.saliency_patch_size], x[:, self.saliency_patch_size:-1]
        rec_patches = self.img_decoder(rec_patches)  # (b, n_image, 3 * t * patch_size ** 2)
        rec_saliency_patches = self.img_decoder(rec_saliency_patches)  # (b, (t n), 2 * patch_size)
        ...
        return rec_saliency_patches, rec_patches

    def reconstruct(self, obs, saliency_obs, task_emb, p_img):
        """
        ...
        task_emb: (b, e)
        """
        assert len(obs.shape) == 5  # b, t, c, h, w
        # track = self._preprocess_track(track)
        obs = self._preprocess_vid(obs)
        saliency_obs = self._preprocess_vid(saliency_obs)
        return self.forward(obs, saliency_obs, task_emb, p_img)
    ...
    def forward_loss(self,
        ...
        # track = self._preprocess_track(track)
        obs = self._preprocess_vid(obs)
        # vis = self._preprocess_vid(vis)

        rec_saliency_patches, rec_patches = self.forward(obs, saliency_obs, task_emb, p_img)
        print(f"check rec shape {rec_saliency_patches.shape}")
        print(f"check rec shape {rec_patches.shape}")
        # vis[vis == 0] = .1
        # vis = repeat(vis, "b tl n -> b tl n c", c=2)

        # track_loss = torch.mean((rec_track - track) ** 2 * vis)
        rec_saliency_patches.to("cuda")
        rec_patches.to("cuda")
        obs.to("cuda")
        saliency_obs.to("cuda")
        patched_saliency_obs = self._patchify(saliency_obs).to("cuda")
        img_loss = torch.mean((rec_patches - self._patchify(obs)) ** 2)
        saliency_loss = torch.mean((rec_saliency_patches - patched_saliency_obs) ** 2)
        loss = lbd_track * saliency_loss + lbd_img * img_loss
        ...
        if return_outs:
            return loss.sum(), ret_dict, (rec_saliency_patches, rec_patches)
        return loss.sum(), ret_dict

    def forward_vis(self, obs, saliency_obs, task_emb, p_img):
        """
            obs: b*v(b) t c h w
            saliency_obs: b*v(b) t c h w
        """
        b = obs.shape[0]
        ...
        rec_saliency, rec_patches = self.forward(obs, saliency_obs, task_emb, p_img)
        saliency_loss = F.mse_loss(rec_saliency, self._patchify(saliency_obs))
        img_loss = F.mse_loss(rec_patches, self._patchify(obs))
        loss = saliency_loss + img_loss

        rec_image = self._unpatchify(rec_patches)
        rec_saliency = self._unpatchify(rec_saliency)

        # place them side by side
        combined_image = torch.cat([obs[:, -1], rec_image[:, -1]], dim=-1)  # only visualize the current frame
        ...
        combined_saliency = torch.cat([saliency_obs[:, -1], rec_saliency[:, -1]], dim=-1)  # only visualize the current frame
        combined_saliency = self.img_unnormalizer(combined_saliency) * 255
        combined_saliency = torch.clamp(combined_saliency, 0, 255)
        combined_saliency = rearrange(combined_saliency, '1 c h w -> h w c')
        ...
        ret_dict = {
            "loss": loss.sum().item(),
            "saliency_loss": saliency_loss.sum().item(),
            "img_loss": img_loss.sum().item(),
            "combined_image": combined_image.cpu().numpy().astype(np.uint8),
            "combined_saliency": combined_saliency.cpu().numpy().astype(np.uint8),
        }
